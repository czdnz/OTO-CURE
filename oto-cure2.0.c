...
//alkol bağımlılığını ölçen test
void alkol_test() {
...
//sigara bağımlılığını ölçen test
void sigara_test() {
...
//günlük kalori ihtiyacını hesaplayan program (boyu santimetre cinsinden girdirin)
void gunluk_kolori() {
...
void depresyondan_kurtulma() {
	...
	printf("*Uyandıktan sonra mumkun oldugunca cabuk kalkın\n");
...
//depresyon durumunu hesaplayan test (hatayı belirt) 
void depresyon_test() {
	...
	printf("3-Eskiden sinirlendigim seylere bile artık sinirlenemiyorum.\n\n");
	scanf_s("%d", &x9);
	if (x9 == 1)
		x9 = 0;
	else if (x9 == 2)
...
// kayıt dosyasina yapılan işlem yerine işlem sonucu yazılacak.
// kayıt dosyasina ad soyad verisi girilebilir.
// hastalık sayısı ve işlemleri arttırılacak
